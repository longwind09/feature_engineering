...
import org.felix.ml.fe.discret.service.impl.ListWithCacheFeatureIndexServiceImpl;
...
public class Process {
    ...
    private ListWithCacheFeatureIndexServiceImpl service;
    ...
    private void setup() throws FileNotFoundException, IOException {
        ...
        service = new ListWithCacheFeatureIndexServiceImpl();
        try {
                ...
                try {
                    if (totalNum % mod == 0) {
                        info.info(String.format(
                                "process at:%s warn:%s,%s pass:%s feats:%s hit_num:%s,%s,%s mis:%s hit_l1:%.2f hit_l2:%.2f hit_l3:%.2f warnNumMap:%s",
                                totalNum, warnNum, warnNumFeats, passNum, service.featsNum, service.cache_L1_Num,
                                service.cache_L2_Num, service.cache_L3_Num, service.miss,
                                service.featsNum > 0 ? service.cache_L1_Num * 100.0f / service.featsNum : -99.0,
                                service.featsNum > 0 ? service.cache_L2_Num * 100.0f / service.featsNum : -99.0,
                                service.featsNum > 0 ? service.cache_L3_Num * 100.0f / service.featsNum : -99.0,
                                warnNumMap.size()));
                        System.gc();
